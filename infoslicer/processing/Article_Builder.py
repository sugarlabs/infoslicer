...
def get_article_from_dita(image_path, dita):
    ...
    try:
        ...
        if imglist_tag is not None:
            for img in imglist_tag.findAll("image"):
                ...
                if caption is not None:
                    caption = caption.renderContents().decode('utf-8').replace("\n", "").strip()
                else:
                    caption = ""
        ...
        logger.error("Failed to parse DITA: %s", str(e))
        raise
...
def get_dita_from_article(image_path, article):
    ...
    try:
        ...
        logger.error("Failed to generate DITA: %s", str(e))
        raise
...
def _tag_generator(soup, name, attrs=None, contents=None):
    ...
    try:
        ...
        logger.error("Failed to generate tag: %s", str(e))
