...
import logging
import random
from infoslicer.processing.article_data import ParagraphData, SectionData, SentenceData
from infoslicer.processing.paragraph import Paragraph, dummyParagraph
from gi.repository import GdkPixbuf


logger = logging.getLogger('infoslicer:Section')
...
class RawSection:
    ...
    def splitParagraph(self, lociter):
        ...
        if firstdata != [] and seconddata != []:
            ...
            paragraphdata = ParagraphData(None, source_article_id, source_section_id, source_paragraph_id, firstdata)
            paragraph = Paragraph(paragraphdata, self.buf, insertioniter)
            self.paragraphs.insert(paragraphindex, paragraph)

            insertioniter = self.buf.get_iter_at_mark(mark)
            paragraphdata = ParagraphData(None, source_article_id, source_section_id, source_paragraph_id, seconddata)
            paragraph = Paragraph(paragraphdata, self.buf, insertioniter)
    ...
    def getData(self):
        ...
        data = SectionData(id, source_article_id, source_section_id, paragraphs_data)
        return data
    ...
    def getDataRange(self, startiter, enditer):
        ...
        if startindex == endindex:
            ...
            if startiter.compare(startparagraph.getStart()) == 0:
                ...
                dummydata = SentenceData(idz = -1, text = "")
    ...
    def pad(self):
        ...
        dummydata = SentenceData(idz = -1, text = " ")
        dummyparagraphdata = ParagraphData(idz = -1, sentences_data = [dummydata])
    ...
    def clean(self):
        ...
        # Returns true if, after removing the pad, the section has no meaningful
        #  content and can therefore be destroyed
        if len(self.paragraphs) > 1:
...
class Section( RawSection ):
    ...
    def __init__(self, section_data, buf, insertioniter):
        ...
        insertioniter = buf.get_iter_at_mark(insertionmark)
        endparagraphdata = ParagraphData(idz = 1, sentences_data = [])
